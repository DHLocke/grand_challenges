...
tic(); trees <-
  ...
  filter(grade_group %in% c("AB","C","D")) |> 
  rowid_to_column(var = 'Burghardt_ID'); toc() #takes 1 min to run
...
trees |> 
  ...
  mapview(filter(holc, grade_group != "industrial"), zcol = "grade_group")
...
trees |> 
  ...
  st_write(paste0('../grand_challenges_large/candidate_street_trees_', Sys.Date(), '.gpkg'))
