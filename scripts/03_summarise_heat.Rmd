...
holc |> 
  left_join(heat_summary, by = 'area_id') |> 
  mapview(zcol = 'mean_temp_C', legend = FALSE)
...
## A points for 2024 and 2025 tree list
```{r}
...
trees_sf <-
  trees_sf |> 
  # move this up to right under st_transform(crs = st_crs(heat))?
...
## B 15m buffers for 2024 and 2025 tree list
```{r}
## With a 15m buffer around the tree
st_crs(trees_sf)$units # check units
trees_15m_buffered <- st_buffer(trees_sf, dist = 15) #make 15 meter buffer

# sanity checks
trees_15m_buffered |> glimpse()
trees_15m_buffered |>  mapview(zcol = 'SPP') + mapview(heat)

weird <- trees_15m_buffered |>
  filter(Burghardt_ID %in% c("124622", "150263", "35655", "37645", "108341"))
weird |> mapview(zcol = 'SPP') + mapview(heat)
...
# 2x check
shp |> st_drop_geometry() |> tabyl(group)
shp |> tidylog::distinct(Burghardt_ID, group) # no rows removed, good
...
for(k in LETTERS[1:n_jobs]){
  job::job({
    ...
    for(i in shp |> filter(group == k) |> distinct(Burghardt_ID) |> pull(Burghardt_ID)){
      ...
      # Output is returned as a long-form data frame with a column for plot ID, pixel class, and pixel count.
      # Error catching code is included so that if there are no values in the circle (entirely outside raster)
      data.frame(polygon_id = i, buffer_15m_temp_C = as.numeric(out)) |>
        data.table::fwrite(paste0('output_tables/heat_15m_buffer_', Sys.Date(), '.csv') # PAY ATTENTION
                           , append = TRUE) # vital!
...
(test <- read_csv('output_tables/heat_15m_buffer_2026-02-16.csv', skip = 1))

nrow(shp); nrow(test) # that's good, same length

# do they join cleanly?
test |> tidylog::left_join(shp, by = c('polygon_id' = 'Burghardt_ID')) # perfectly. Maybe change 'polygon_id" to 'Burghard_ID' in the data.frame write out step?

test |> tidylog::anti_join(shp, by = c('polygon_id' = 'Burghardt_ID')) # perfect.

# Okay let's try those weird trees that didn't run
n_jobs <- 3
shp <-
  weird |>
  tidylog::select(Burghardt_ID) |>
...
for(k in LETTERS[1:n_jobs]){
  job::job({
    ...
    for(i in shp |> filter(group == k) |> distinct(Burghardt_ID) |> pull(Burghardt_ID)){
      ...
      # Error catching code is included so that if there are no values in the circle (entirely outside raster)
      data.frame(polygon_id = i, buffer_15m_temp_C = as.numeric(out)) |>
        data.table::fwrite(paste0('output_tables/weird_15m_buffer_', Sys.Date(), '.csv') # PAY ATTENTION
                           , append = TRUE) # vital!
...
## C 50m buffers for 2024 and 2025 tree list
```{r}
## With a 50m buffer around the tree
st_crs(trees_sf)$units # check units
trees_50m_buffered <- st_buffer(trees_sf, dist = 50) #make 50 meter buffer

# sanity checks
trees_50m_buffered |> glimpse()
trees_50m_buffered |>  mapview(zcol = 'SPP') + mapview(heat)
...
for(k in LETTERS[1:n_jobs]){
  job::job({
    ...
    for(i in shp |> filter(group == k) |> distinct(Burghardt_ID) |> pull(Burghardt_ID)){
      ...
      # Output is returned as a long-form data frame with a column for plot ID, pixel class, and pixel count.
      # Error catching code is included so that if there are no values in the circle (entirely outside raster)
      data.frame(polygon_id = i, buffer_50m_temp_C = as.numeric(out)) |>
        data.table::fwrite(paste0('output_tables/heat_50m_buffer_', Sys.Date(), '.csv') # PAY ATTENTION
                           , append = TRUE) # vital!
...
### i read buffer summaries back in
```{r eval=FALSE, include=FALSE}
# did that work?
(heat_15m_buffer <- read_csv('output_tables/heat_15m_buffer_2026-02-12.csv') |> 
    rename(Burghardt_ID = polygon_id))

# a double check
heat_15m_buffer |> 
  distinct(Burghardt_ID) |> 
  nrow() - shp |> 
  nrow() # 3 missing
...
# does this join back cleanly?
trees_15m_buffered |> 
  left_join(heat_15m_buffer, by = 'Burghardt_ID')

trees_15m_buffered |> 
  left_join(heat_15m_buffer, by = 'Burghardt_ID') |> 
  mapview(zcol = 'buffer_15m_temp_C')

# Write out all the buffered data
trees_15m_buffered<-
  left_join(trees_15m_buffered,heat_15m_buffer, by = 'Burghardt_ID') |>
  st_drop_geometry()

write_csv(trees_15m_buffered,paste0('../grand_challenges/output_tables/15m_buffer_', Sys.Date(), '.csv'),append=F)

```


## 5 buffer for 2025 tree list
```{r}
