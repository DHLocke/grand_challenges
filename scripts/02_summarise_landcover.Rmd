...
## C Buffers for 2024 and 2025 tree list
```{r}
...
# sanity checks
trees_sf |> glimpse()
trees_sf |> mapview(zcol = 'SPP') + mapview(lcc)
...
trees_sf_buffered |> mapview(zcol = 'SPP') + mapview(lcc)

# extract percent tree cover from raster to buffer
n_jobs <- 3
shp <-
  trees_sf_buffered |>
...
for(k in LETTERS[1:n_jobs]){
  job::job({
    ...
    for(i in shp |> filter(group == k) |> distinct(Burghardt_ID) |> pull(Burghardt_ID)){
      ...
      # the work horse of the whole operation
      out <-
        terra::crop(lcc, shp[shp$Burghardt_ID == i,]) |>
      ...
      if (length(out) > 0) {
        data.frame(polygon_id = i, class = names(out), pixel_count = as.numeric(out)) |>
          data.table::fwrite(paste0('output_tables/lcc_summary_buffer', Sys.Date(), '.csv') # PAY ATTENTION
                    , append = TRUE) # vital!
        } else {
          data.frame(polygon_id = i, class = character(0), pixel_count = numeric(0)) |> 
            data.table::fwrite(paste0('output_tables/lcc_summary_buffer', Sys.Date(), '.csv') # PAY ATTENTION
